...
class Executable_Job(Job):
    ...
    def __init__(self, requests, job_id, subtask_id, depends_on, user, server_address):
        super().__init__(requests, job_id, subtask_id, depends_on, user)
    ...
    def initialize_qlog(self):
        ...
        try:
            if not os.path.exists("q.log"):
                os.makedirs("q.log")
            self.qlog = open(
                "q.log/" + self.name + "." + str(self.job_id).zfill(7), "w"
            )
        except:
            self.qlog = None
    ...
    def finalize(self):
        ...
        self.write_log(
            "elapsed time: %s:%s:%s"
            % (
                str(elapsed.days * 24 + (elapsed.seconds / 3600)).zfill(3),
    ...
    def run(self, script_params=[]):
        ...
        # memory limit
        if (
            self.requests["gpus"] == 0
        ):  # ulimit does not work in combination with GPUs due to unified memory
...
def main():
    ...
    arg_parser.add_argument(
        "--block_index", type=int, default=0, help="index of the block to execute"
    )
    arg_parser.add_argument(
        "--job_id", type=int, default=0, help="unique id of this job"
    )
    arg_parser.add_argument(
        "--subtask_id", type=int, default=1, help="subtask id for the selected block"
    )
    arg_parser.add_argument(
        "--depends_on",
        type=str,
        default="",
        help="comma separated list of job ids that need to be finished before this job starts",
    )
    arg_parser.add_argument(
        "--user", type=str, default="dummy", help="user who submitted the job"
    )
    ...
    job = Executable_Job(
        blocks[args.block_index].values,
        args.job_id,
        args.subtask_id,
        depends_on,
        args.user,

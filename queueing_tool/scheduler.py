...
class SchedulerJob:
    ...
    def __init__(self, job, job_id):  # job is a Job object from the server
        ...
        self.time = max(
            1, (datetime.datetime.now() - job.time).total_seconds()
        )  # waiting time/running time of the job
        self.priority = job.priority
        self.job_id = job_id

    def fits(self, resources):
        return (
            (self.resources.gpus <= resources.gpus)
            and (self.resources.threads <= resources.threads)
            and (self.resources.memory <= resources.memory)
        )
...
class Scheduler:
    ...
    def schedule(self, jobs, waiting_ids, running_ids):
        try:
            waiting_jobs = [
                SchedulerJob(jobs[job_id], job_id) for job_id in waiting_ids
            ]
            ...
            priorities, job_ids, waiting_jobs = list(
                zip(
                    *sorted(
                        zip(priorities, job_ids, waiting_jobs),
            ...
            if waiting_jobs[0].fits(self.free_resources):
                ...
                running_jobs = [
                    SchedulerJob(jobs[job_id], job_id) for job_id in running_ids
                ]

...
def main():
    ...
    arg_parser.add_argument(
        ...
        type=str,
        nargs="+",
        help="Jobs to be deleted. Is a space separated list of job names, user names, or job ids. For ids, job ranges separated by a '-' are also possible.",
    )
    ...
    group.add_argument(
        "-n",
        action="store_true",
        help="Delete all jobs of the given names. Asterisks can be used as wildcards.",
    )
    group.add_argument(
        "-u",
        action="store_true",
        help="Delete all jobs of the given users. Asterisks can be used as wildcards.",
    )
            ...
            for job in args.jobs:
                if len(job.split("-")) == 2:
                    joblist += [
                        i
                        for i in range(
                            int(job.split("-")[0]), int(job.split("-")[1]) + 1
                        )
                    ]
                else:
    ...
    try:
        ...
        for job in joblist:
            ...
            while reply != ":" and reply != "":
                sock.sendall(b":")  # send acknowledgement to server

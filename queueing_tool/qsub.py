...
def build_job_command(
    script, server_address, block_idx, job_id, subtask_id, depends_on, user
):
    # Spawn the job runner through the interpreter, never a bare PATH name —
    # 'job.py' resolved via PATH was the classic broken-install failure mode.
    cmd = [
        sys.executable,
        "-m",
        "queueing_tool.job",
    ...
    if len(depends_on) > 0:
        cmd.append("--depends_on=" + ",".join([str(i) for i in depends_on]))
    return cmd + list(script)
...
def submit_job(script, server_address, block_idx, job_id, subtask_id, depends_on):
    ...
    # detach so the runner outlives qsub and the terminal (was os.system + ' &')
...
def main():
    ...
    arg_parser.add_argument(
        "-l", "--local", action="store_true", help="Execute the script locally"
    )
    ...
    group.add_argument(
        ...
        type=str,
        default="",
        help="Only submit/execute the specified block",
    )
    group.add_argument(
        "-s",
        "--subtask",
        ...
        default="",
        nargs=2,
        help="Arguments for this option are a block name and a subtask id."
    ...
    group.add_argument(
        ...
        type=str,
        default="",
        help="Submit/execute the specified block and all succeeding blocks",
    )
    arg_parser.add_argument(
    ...
    if not args.block == "":
        ...
        try:
            ...
            print(
                "block "
                + args.subtask[0]
                + ": subtask id must be in the range 1,...,"
                + str(blocks[block_indices[0]].values["subtasks"])
                + " but is "
                + args.subtask[1]
            )
            exit()
    ...
    if args.local:
        ...
        for block_idx in block_indices:
            subtask_ids = (
                list(range(blocks[block_idx].values["subtasks"]))
                if args.subtask == ""
                else [int(args.subtask[1]) - 1]
            )

...
def build_job_command(script, server_address, block_idx, job_id, subtask_id, depends_on, user):
    ...
    cmd = [sys.executable, '-m', 'queueing_tool.job',
...
def submit_job(script, server_address, block_idx, job_id, subtask_id, depends_on):
    cmd = build_job_command(script, server_address, block_idx, job_id, subtask_id,
                            depends_on, os.getenv('USER'))
